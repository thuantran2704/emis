const aboutContent = {
  english: {
    metaTitle: 'About EMIS — Implant & Full-Mouth Rehabilitation',
    heroTitle: 'Advanced Implant & Full-Mouth Rehabilitation Care',
    heroSubtitle:
      'At EMIS, we help patients restore comfort, function, and confidence through carefully planned implant and rehabilitation treatment. Whether you live in Vietnam or overseas, our goal is to help you understand your situation clearly and explore treatment options with greater confidence.',
    heroPrimaryCTA: 'Send Your X-ray',
    heroSecondaryCTA: 'Meet Our Doctors',

    approachTitle: 'Treatment Begins With Understanding',
    approachText:
      'Every patient comes with different concerns, goals, and expectations. Before discussing treatment options, we focus on understanding your condition, listening to your concerns, and helping you see what may be appropriate for your situation. We believe confident decisions begin with clear understanding.',

    planningTitle: 'Careful Planning Before Treatment',
    planningBullets: [
      {
        title: 'Clinical Assessment',
        text: 'Understanding your current oral condition and treatment needs.',
      },
      {
        title: '3D Imaging Evaluation',
        text: 'Evaluating anatomy, bone structure, and clinical considerations.',
      },
      {
        title: 'Treatment Planning',
        text: 'Creating a personalized treatment pathway.',
      },
      {
        title: 'Long-Term Considerations',
        text: 'Planning for function, stability, and long-term outcomes.',
      },
    ],

    technologyTitle: 'Supporting Better Decisions',
    technologyIntro: 'Modern technology helps clinicians evaluate, plan, and deliver treatment with greater clarity and precision. Patient safety and treatment quality remain central to every stage of care.',
    technologyBullets: ['3D CBCT Imaging', 'Digital Treatment Planning', 'Dedicated Implant Room', 'Sterilization Protocols'],
    technologyImageAlt: '3D CBCT imaging and clinical implant environment',

    teamTitle: 'The Team Behind Your Care',
    teamIntro: 'A team working toward one goal: helping patients regain comfort, function, confidence, and long-term quality of life.',
    teamMembers: [
      {
        name: 'Dr. Tran Minh Tu',
        role: 'Odonto-Maxillofacial Surgeon',
        description: 'Implant Surgery • Oral Surgery • Full-Mouth Rehabilitation. Currently practicing at Ho Chi Minh City Odonto-Maxillofacial Hospital and EMIS Premium Dental Clinic.',
        cta: 'Contact Us',
      },
      {
        name: 'Dr. Nguyen The Son',
        role: 'Chief Dentist - Implant & Aesthetic Prosthetics',
        description: 'Dr. Son has over 10 years of specialized experience in dental implants, aesthetic prosthetics, and sinus lift - bone grafting procedures. With more than 3,000 implant cases from simple to complex, he focuses on restoring chewing function, natural esthetics, and long-term treatment stability.',
        cta: 'Contact Us',
      },
    ],

    internationalTitle: 'Support Before, During, And After Treatment',
    internationalIntro: 'Many patients travel to Vietnam specifically for implant or rehabilitation treatment. To help make the process clearer and more manageable, EMIS provides support before, during, and after your visit.',
    internationalBullets: [
      'Online Consultation',
      'Preliminary X-ray Review',
      'Treatment Timeline Planning',
      'Remote Follow-Up Support',
      'Language Assistance When Needed',
    ],

    trustTitle: 'Building Confidence Through Clarity',
    trustBullets: [
      'Clinic License Information',
      'Doctor Profiles & Credentials',
      'Verified Google Maps Location',
      'Official Communication Channels (WhatsApp, Email)',
    ],

    startTitle: 'What Happens Next?',
    steps: [
      { step: 'Step 1', title: 'Send Your X-ray', text: 'Send your X-ray so we can perform a preliminary assessment.' },
      { step: 'Step 2', title: 'Receive A Preliminary Assessment', text: 'We provide an initial review and outline possible options.' },
      { step: 'Step 3', title: 'Discuss Your Treatment Options', text: 'A detailed discussion to help you choose an appropriate pathway.' },
    ],
    note: 'A preliminary assessment helps us better understand your situation. Final treatment recommendations require an in-person examination and appropriate diagnostic imaging.',
    startPrimaryCTA: 'Send Your X-ray',
    startSecondaryCTA: 'Talk With Our Team',
  },

  vietnamese: {
    metaTitle: 'Về EMIS — Implant & Phục Hồi Toàn Hàm',
    heroTitle: 'Chuyên Sâu Implant & Phục Hồi Toàn Hàm',
    heroSubtitle:
      'Tại EMIS, chúng tôi tập trung vào Implant và phục hồi toàn hàm nhằm giúp bệnh nhân khôi phục khả năng ăn nhai, sự tự tin và chất lượng cuộc sống lâu dài. Dù bạn đang sinh sống tại Việt Nam hay ở nước ngoài, chúng tôi mong muốn giúp bạn hiểu rõ sức khỏe răng miệng của mình và có đủ thông tin trước khi đưa ra lựa chọn.',
    heroPrimaryCTA: 'Gửi X-ray Của Bạn',
    heroSecondaryCTA: 'Tìm Hiểu Đội Ngũ Bác Sĩ',

    approachTitle: 'Điều Trị Bắt Đầu Từ Sự Thấu Hiểu',
    approachText:
      'Mỗi bệnh nhân đều có một mối quan tâm, nhu cầu và mục tiêu khác nhau. Trước khi xây dựng kế hoạch điều trị, chúng tôi dành thời gian để lắng nghe, đánh giá kỹ lưỡng và giúp bệnh nhân hiểu rõ những vấn đề mình đang gặp phải. Chúng tôi tin rằng những quyết định đúng đắn luôn bắt đầu từ sự thấu hiểu.',

    planningTitle: 'Đánh Giá Kỹ Trước Khi Điều Trị',
    planningBullets: [
      { title: 'Đánh Giá Tình Trạng Hiện Tại', text: 'Đánh giá sức khỏe răng miệng hiện tại và những nhu cầu cần được giải quyết.' },
      { title: 'Chẩn Đoán Hình Ảnh 3D', text: 'Đánh giá cấu trúc xương, giải phẫu và các yếu tố chuyên môn liên quan.' },
      { title: 'Lập Kế Hoạch Điều Trị', text: 'Xây dựng hướng điều trị dựa trên từng trường hợp cụ thể.' },
      { title: 'Mục Tiêu Dài Hạn', text: 'Hướng tới khả năng ăn nhai ổn định, thẩm mỹ tự nhiên và kết quả lâu dài.' },
    ],

    technologyTitle: 'Công Nghệ & An Toàn',
    technologyIntro: 'Công nghệ giúp quá trình đánh giá, lập kế hoạch và điều trị trở nên chính xác và dễ dự đoán hơn. Sự an toàn và chất lượng điều trị luôn được đặt lên hàng đầu trong mọi giai đoạn chăm sóc.',
    technologyBullets: ['Chẩn Đoán Hình Ảnh CBCT 3D', 'Lập Kế Hoạch Điều Trị Bằng Công Nghệ Số', 'Phòng Implant Riêng Biệt', 'Quy Trình Vô Trùng & Kiểm Soát Nhiễm Khuẩn'],

    teamTitle: 'Đội Ngũ Đồng Hành Cùng Bạn',
    teamIntro: 'Một đội ngũ hướng tới một mục tiêu chung: giúp bệnh nhân khôi phục khả năng ăn nhai, tự tin và chất lượng cuộc sống lâu dài.',
    teamMembers: [
      { name: 'BS. CKI Trần Minh Tú', role: 'Phẫu thuật Răng Hàm Mặt', description: 'Implant • Tiểu phẫu Răng Hàm Mặt • Phục hồi toàn hàm. Hiện đang công tác tại Bệnh viện Răng Hàm Mặt TP.HCM và EMIS Premium Dental Clinic.', cta: 'Liên Hệ Chúng Tôi' },
      { name: 'BS. CKI Nguyễn Thế Sơn', role: 'Bác Sĩ Trưởng - Implant & Phục Hình Thẩm Mỹ', description: 'Bác sĩ Sơn có hơn 10 năm kinh nghiệm chuyên sâu trong lĩnh vực cấy ghép Implant, phục hình thẩm mỹ và phẫu thuật nâng xoang - ghép xương. Với hơn 3.000 ca Implant từ đơn giản đến phức tạp, bác sĩ tập trung phục hồi chức năng ăn nhai, thẩm mỹ tự nhiên và sự ổn định lâu dài.', cta: 'Liên Hệ Chúng Tôi' },
    ],

    internationalTitle: 'Hỗ Trợ Trước, Trong Và Sau Điều Trị',
    internationalIntro: 'Nhiều bệnh nhân lựa chọn trở về Việt Nam để làm Implant hoặc phục hồi toàn hàm. Chúng tôi cố gắng giúp quá trình này trở nên thuận tiện và rõ ràng hơn thông qua việc chuẩn bị trước và hỗ trợ xuyên suốt hành trình điều trị.',
    internationalBullets: ['Tư Vấn Trực Tuyến', 'Đánh Giá X-ray Ban Đầu', 'Lập Kế Hoạch Theo Thời Gian Lưu Trú', 'Theo Dõi Từ Xa Sau Điều Trị', 'Hỗ Trợ Ngôn Ngữ Khi Cần'],

    trustTitle: 'Minh Bạch & Đáng Tin Cậy',
    trustBullets: ['Giấy Phép Hoạt Động', 'Hồ Sơ & Bằng Cấp Bác Sĩ', 'Google Maps & Địa Chỉ Phòng Khám', 'Kênh Liên Hệ Chính Thức (WhatsApp, Email)'],

    startTitle: 'Bắt Đầu Từ Việc Hiểu Rõ Trường Hợp Của Bạn',
    steps: [
      { step: 'Bước 1', title: 'Gửi X-ray', text: 'Gửi X-ray hoặc chia sẻ thông tin về trường hợp của bạn.' },
      { step: 'Bước 2', title: 'Nhận đánh giá sơ bộ', text: 'Nhận đánh giá sơ bộ từ đội ngũ EMIS.' },
      { step: 'Bước 3', title: 'Trao đổi về lựa chọn điều trị', text: 'Cùng trao đổi về những lựa chọn có thể phù hợp với nhu cầu và mong muốn của bạn.' },
    ],
    note: 'Đánh giá ban đầu giúp chúng tôi hiểu rõ hơn về trường hợp bạn. Kế hoạch điều trị cuối cùng sẽ được xác định sau khi thăm khám trực tiếp và thực hiện đầy đủ các chẩn đoán cần thiết.',
    startPrimaryCTA: 'Gửi X-ray Của Bạn',
    startSecondaryCTA: 'Trao Đổi Với Đội Ngũ EMIS',
  },

  french: {
    metaTitle: 'À Propos d\'EMIS — Implantologie & Réhabilitation Complète',
    heroTitle: 'Soins Avancés en Implantologie et Réhabilitation Complète',
    heroSubtitle:
      'Chez EMIS, nous aidons les patients à retrouver confort, fonction et confiance grâce à une prise en charge implantaire et de réhabilitation soigneusement planifiée. Que vous viviez au Vietnam ou à l\'étranger, notre objectif est de vous aider à comprendre clairement votre situation et à explorer vos options avec confiance.',
    heroPrimaryCTA: 'Envoyez Votre Radiographie',
    heroSecondaryCTA: 'Rencontrez Nos Médecins',

    approachTitle: 'Le Traitement Commence Par La Compréhension',
    approachText:
      'Chaque patient a des préoccupations, des objectifs et des attentes différents. Avant de discuter des options de traitement, nous cherchons d\'abord à comprendre votre état, écouter vos besoins et vous aider à identifier les solutions adaptées. Nous croyons que les bonnes décisions commencent par une compréhension claire.',

    planningTitle: 'Planification Soignée Avant Le Traitement',
    planningBullets: [
      {
        title: 'Évaluation Clinique',
        text: 'Comprendre votre état bucco-dentaire actuel et vos besoins de traitement.',
      },
      {
        title: 'Évaluation Par Imagerie 3D',
        text: 'Évaluer l\'anatomie, la structure osseuse et les considérations cliniques.',
      },
      {
        title: 'Planification Du Traitement',
        text: 'Créer un parcours de traitement personnalisé.',
      },
      {
        title: 'Considérations À Long Terme',
        text: 'Planifier la fonction, la stabilité et les résultats durables.',
      },
    ],

    technologyTitle: 'Des Décisions Mieux Éclairées',
    technologyIntro: 'Les technologies modernes aident les cliniciens à évaluer, planifier et réaliser les traitements avec plus de clarté et de précision. La sécurité du patient et la qualité des soins restent au centre de chaque étape.',
    technologyBullets: ['Imagerie 3D CBCT', 'Planification Numérique Du Traitement', 'Salle D\'Implantologie Dédiée', 'Protocoles De Stérilisation'],
    technologyImageAlt: 'Imagerie CBCT 3D et environnement clinique implantaire',

    teamTitle: 'L\'Équipe Derrière Vos Soins',
    teamIntro: 'Une équipe tournée vers un objectif: aider les patients à retrouver confort, fonction, confiance et qualité de vie à long terme.',
    teamMembers: [
      {
        name: 'Dr Tran Minh Tu',
        role: 'Chirurgien Maxillo-Facial',
        description: 'Chirurgie implantaire • Chirurgie orale • Réhabilitation complète. Exerce actuellement à l\'Hôpital Odonto-Maxillo-Facial de Ho Chi Minh-Ville et à EMIS Premium Dental Clinic.',
        cta: 'Contactez-Nous',
      },
      {
        name: 'Dr Nguyen The Son',
        role: 'Dentiste Principal - Implantologie & Prothèse Esthétique',
        description: 'Le Dr Son possède plus de 10 ans d\'expérience spécialisée en implants dentaires, prothèses esthétiques et chirurgie de comblement osseux avec élévation sinusienne. Avec plus de 3 000 cas d\'implantologie, il se concentre sur la restauration de la fonction masticatoire, de l\'esthétique naturelle et de la stabilité à long terme.',
        cta: 'Contactez-Nous',
      },
    ],

    internationalTitle: 'Accompagnement Avant, Pendant Et Après Le Traitement',
    internationalIntro: 'De nombreux patients voyagent au Vietnam pour des traitements implantaires ou de réhabilitation. Pour rendre le processus plus clair et plus simple, EMIS fournit un accompagnement avant, pendant et après votre visite.',
    internationalBullets: [
      'Consultation En Ligne',
      'Évaluation Préliminaire De La Radiographie',
      'Planification Du Calendrier De Traitement',
      'Suivi À Distance',
      'Assistance Linguistique Si Nécessaire',
    ],

    trustTitle: 'Construire La Confiance Par La Clarté',
    trustBullets: [
      'Informations Sur La Licence De La Clinique',
      'Profils Et Qualifications Des Médecins',
      'Localisation Vérifiée Sur Google Maps',
      'Canaux Officiels De Communication (WhatsApp, Email)',
    ],

    startTitle: 'Que Se Passe-T-Il Ensuite?',
    steps: [
      { step: 'Étape 1', title: 'Envoyez Votre Radiographie', text: 'Envoyez votre radiographie afin que nous puissions effectuer une évaluation préliminaire.' },
      { step: 'Étape 2', title: 'Recevez Une Évaluation Préliminaire', text: 'Nous vous fournissons une première analyse et des options possibles.' },
      { step: 'Étape 3', title: 'Discutez De Vos Options', text: 'Une discussion détaillée pour vous aider à choisir la voie adaptée.' },
    ],
    note: 'Une évaluation préliminaire nous aide à mieux comprendre votre situation. Les recommandations finales nécessitent un examen en personne et des examens diagnostiques appropriés.',
    startPrimaryCTA: 'Envoyez Votre Radiographie',
    startSecondaryCTA: 'Parler Avec Notre Équipe',
  },

  simplified: {
    metaTitle: '关于 EMIS — 种植与全口重建',
    heroTitle: '先进种植与全口重建护理',
    heroSubtitle:
      '在 EMIS，我们通过周密规划的种植与重建治疗，帮助患者恢复舒适、功能与自信。无论您居住在越南还是海外，我们都希望帮助您清晰了解自身情况，并更有信心地选择治疗方案。',
    heroPrimaryCTA: '发送您的 X 光片',
    heroSecondaryCTA: '了解医生团队',

    approachTitle: '治疗始于理解',
    approachText:
      '每位患者的顾虑、目标与期望都不同。在讨论治疗方案前，我们首先重视对病情的理解、倾听您的需求，并帮助您看到更适合自身情况的方向。我们相信，可靠的决定始于清晰的理解。',

    planningTitle: '治疗前的严谨规划',
    planningBullets: [
      {
        title: '临床评估',
        text: '了解您当前的口腔状况与治疗需求。',
      },
      {
        title: '3D 影像评估',
        text: '评估解剖结构、骨条件及临床相关因素。',
      },
      {
        title: '治疗规划',
        text: '制定个性化治疗路径。',
      },
      {
        title: '长期考量',
        text: '规划功能、稳定性与长期效果。',
      },
    ],

    technologyTitle: '支持更清晰的决策',
    technologyIntro: '现代技术帮助医生更清晰、更精准地评估、规划与实施治疗。患者安全与治疗质量始终贯穿每个阶段。',
    technologyBullets: ['3D CBCT 影像', '数字化治疗规划', '独立种植手术室', '灭菌流程'],
    technologyImageAlt: '3D CBCT 影像与种植临床环境',

    teamTitle: '守护您治疗的团队',
    teamIntro: '一个共同目标明确的团队：帮助患者恢复舒适、功能、自信与长期生活质量。',
    teamMembers: [
      {
        name: 'Dr. Tran Minh Tu',
        role: '口腔颌面外科医生',
        description: '种植手术 • 口腔外科 • 全口重建。现就职于胡志明市口腔颌面医院及 EMIS Premium Dental Clinic。',
        cta: '联系我们',
      },
      {
        name: 'Dr. Nguyen The Son',
        role: '首席牙医 - 种植与美学修复',
        description: 'Son 医生在牙科种植、美学修复及上颌窦提升与骨移植方面有超过 10 年经验，完成 3,000+ 例从简单到复杂的种植病例，重点关注咀嚼功能恢复、自然美学与长期稳定性。',
        cta: '联系我们',
      },
    ],

    internationalTitle: '治疗前中后的支持服务',
    internationalIntro: '许多患者专程前往越南进行种植或重建治疗。为让过程更清晰、更易管理，EMIS 提供治疗前、中、后的全程支持。',
    internationalBullets: [
      '在线咨询',
      'X 光片初步评估',
      '治疗时间规划',
      '远程复诊支持',
      '按需语言协助',
    ],

    trustTitle: '以透明建立信任',
    trustBullets: [
      '诊所执照信息',
      '医生资历与证书',
      'Google 地图认证地址',
      '官方沟通渠道（WhatsApp、Email）',
    ],

    startTitle: '接下来会发生什么？',
    steps: [
      { step: '步骤 1', title: '发送您的 X 光片', text: '请发送您的 X 光片，我们将进行初步评估。' },
      { step: '步骤 2', title: '获取初步评估', text: '我们会提供初步分析并说明可能方案。' },
      { step: '步骤 3', title: '讨论治疗选项', text: '通过详细沟通帮助您选择更适合的路径。' },
    ],
    note: '初步评估有助于我们更好地了解您的情况。最终治疗建议需在面诊与必要影像检查后确定。',
    startPrimaryCTA: '发送您的 X 光片',
    startSecondaryCTA: '与团队沟通',
  },

  traditional: {
    metaTitle: '關於 EMIS — 植牙與全口重建',
    heroTitle: '進階植牙與全口重建照護',
    heroSubtitle:
      '在 EMIS，我們透過審慎規劃的植牙與重建治療，協助患者恢復舒適、功能與自信。無論您居住在越南或海外，我們都希望幫助您清楚理解自身狀況，並更有信心地評估治療選項。',
    heroPrimaryCTA: '傳送您的 X 光片',
    heroSecondaryCTA: '認識醫療團隊',

    approachTitle: '治療始於理解',
    approachText:
      '每位患者的顧慮、目標與期待都不同。在討論治療方案前，我們先重視病況理解、傾聽需求，並協助您看見適合自身情況的方向。我們相信，好的決策始於清晰理解。',

    planningTitle: '治療前的審慎規劃',
    planningBullets: [
      {
        title: '臨床評估',
        text: '了解您目前的口腔狀況與治療需求。',
      },
      {
        title: '3D 影像評估',
        text: '評估解剖條件、骨質結構與臨床考量。',
      },
      {
        title: '治療規劃',
        text: '建立個人化治療路徑。',
      },
      {
        title: '長期考量',
        text: '規劃功能、穩定性與長期結果。',
      },
    ],

    technologyTitle: '支援更清晰的決策',
    technologyIntro: '現代技術可協助醫師更清楚、精準地評估、規劃與執行治療。患者安全與治療品質始終是每個階段的核心。',
    technologyBullets: ['3D CBCT 影像', '數位化治療規劃', '獨立植牙診療室', '滅菌流程'],
    technologyImageAlt: '3D CBCT 影像與植牙臨床環境',

    teamTitle: '守護您治療的團隊',
    teamIntro: '一個共同目標明確的團隊：協助患者恢復舒適、功能、自信與長期生活品質。',
    teamMembers: [
      {
        name: 'Dr. Tran Minh Tu',
        role: '口腔顎面外科醫師',
        description: '植牙手術 • 口腔外科 • 全口重建。現任職於胡志明市口腔顎面醫院與 EMIS Premium Dental Clinic。',
        cta: '聯絡我們',
      },
      {
        name: 'Dr. Nguyen The Son',
        role: '主治醫師 - 植牙與美學贋復',
        description: 'Son 醫師在牙科植牙、美學贋復及上顎竇提升與補骨手術方面有超過 10 年經驗，完成 3,000+ 例植牙病例，重點在恢復咀嚼功能、自然美觀與長期穩定性。',
        cta: '聯絡我們',
      },
    ],

    internationalTitle: '治療前中後支援',
    internationalIntro: '許多患者會前往越南進行植牙或重建治療。為使流程更清楚、可控，EMIS 提供治療前、中、後的完整支援。',
    internationalBullets: [
      '線上諮詢',
      'X 光片初步評估',
      '治療時程規劃',
      '遠距追蹤支援',
      '需要時提供語言協助',
    ],

    trustTitle: '以清楚透明建立信任',
    trustBullets: [
      '診所執照資訊',
      '醫師資歷與證書',
      'Google 地圖認證位置',
      '官方聯繫管道（WhatsApp、Email）',
    ],

    startTitle: '接下來會如何？',
    steps: [
      { step: '步驟 1', title: '傳送您的 X 光片', text: '請先傳送 X 光片，以便我們進行初步評估。' },
      { step: '步驟 2', title: '取得初步評估', text: '我們會提供初步分析與可能方案。' },
      { step: '步驟 3', title: '討論治療選項', text: '透過詳細溝通協助您選擇適合路徑。' },
    ],
    note: '初步評估可幫助我們更了解您的狀況。最終治療建議仍需現場檢查與必要影像診斷後確認。',
    startPrimaryCTA: '傳送您的 X 光片',
    startSecondaryCTA: '與團隊討論',
  },

  korean: {
    metaTitle: 'EMIS 소개 — 임플란트 및 전악 재건',
    heroTitle: '고도 임플란트 및 전악 재건 치료',
    heroSubtitle:
      'EMIS는 체계적으로 계획된 임플란트 및 재건 치료를 통해 환자의 편안함, 기능, 자신감을 회복하도록 돕습니다. 베트남 거주자와 해외 환자 모두가 자신의 상태를 명확히 이해하고 더 자신 있게 치료 옵션을 선택할 수 있도록 지원합니다.',
    heroPrimaryCTA: 'X-ray 보내기',
    heroSecondaryCTA: '의료진 소개 보기',

    approachTitle: '치료는 이해에서 시작됩니다',
    approachText:
      '환자마다 걱정, 목표, 기대가 다릅니다. 치료 옵션을 논의하기 전에 먼저 상태를 정확히 파악하고, 환자의 우려를 경청하며, 상황에 맞는 방향을 함께 확인합니다. 좋은 결정은 명확한 이해에서 시작된다고 믿습니다.',

    planningTitle: '치료 전 정밀한 계획',
    planningBullets: [
      {
        title: '임상 평가',
        text: '현재 구강 상태와 치료 필요를 파악합니다.',
      },
      {
        title: '3D 영상 평가',
        text: '해부학적 구조, 골 상태, 임상적 고려사항을 평가합니다.',
      },
      {
        title: '치료 계획 수립',
        text: '개인 맞춤형 치료 경로를 설계합니다.',
      },
      {
        title: '장기적 고려',
        text: '기능, 안정성, 장기 결과를 고려해 계획합니다.',
      },
    ],

    technologyTitle: '더 나은 의사결정을 위한 기술',
    technologyIntro: '현대 기술은 의료진이 더 명확하고 정밀하게 평가, 계획, 치료를 수행하도록 돕습니다. 환자 안전과 치료 품질은 모든 단계의 핵심입니다.',
    technologyBullets: ['3D CBCT 영상', '디지털 치료 계획', '전용 임플란트 수술실', '멸균 프로토콜'],
    technologyImageAlt: '3D CBCT 영상 및 임플란트 임상 환경',

    teamTitle: '당신의 치료를 책임지는 팀',
    teamIntro: '하나의 목표를 향한 팀: 환자의 편안함, 기능, 자신감, 장기적 삶의 질 회복.',
    teamMembers: [
      {
        name: 'Dr. Tran Minh Tu',
        role: '구강악안면외과 전문의',
        description: '임플란트 수술 • 구강외과 • 전악 재건. 현재 호치민시 구강악안면병원 및 EMIS Premium Dental Clinic에서 진료 중입니다.',
        cta: '문의하기',
      },
      {
        name: 'Dr. Nguyen The Son',
        role: '총괄 치과의사 - 임플란트 & 심미 보철',
        description: 'Son 원장은 임플란트, 심미 보철, 상악동 거상 및 골이식 분야에서 10년 이상의 전문 경력을 보유하고 있으며, 3,000건 이상의 임플란트 케이스를 통해 저작 기능, 자연스러운 심미성, 장기 안정성 회복에 집중합니다.',
        cta: '문의하기',
      },
    ],

    internationalTitle: '치료 전·중·후 지원',
    internationalIntro: '많은 환자들이 임플란트 또는 재건 치료를 위해 베트남을 방문합니다. 과정을 더 명확하고 수월하게 만들기 위해 EMIS는 방문 전, 중, 후 전반의 지원을 제공합니다.',
    internationalBullets: [
      '온라인 상담',
      'X-ray 사전 검토',
      '치료 일정 계획',
      '원격 사후관리 지원',
      '필요 시 언어 지원',
    ],

    trustTitle: '명확함을 통한 신뢰 구축',
    trustBullets: [
      '클리닉 면허 정보',
      '의료진 프로필 및 자격',
      '검증된 Google 지도 위치',
      '공식 소통 채널(WhatsApp, Email)',
    ],

    startTitle: '다음 단계는?',
    steps: [
      { step: '1단계', title: 'X-ray 보내기', text: '초기 평가를 위해 X-ray를 보내주세요.' },
      { step: '2단계', title: '초기 평가 받기', text: '초기 검토 결과와 가능한 옵션을 안내드립니다.' },
      { step: '3단계', title: '치료 옵션 상담', text: '상세 상담을 통해 적절한 치료 방향을 함께 결정합니다.' },
    ],
    note: '초기 평가는 상태를 이해하는 데 도움이 됩니다. 최종 치료 권고는 내원 검사 및 적절한 진단 영상 후 확정됩니다.',
    startPrimaryCTA: 'X-ray 보내기',
    startSecondaryCTA: '팀과 상담하기',
  },
};

export default aboutContent;
